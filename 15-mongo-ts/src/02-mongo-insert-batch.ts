...
async function main() {
    ...
    try {
        ...
        if (insRes.acknowledged && insRes.insertedCount === MOCK_POSTS.length) {
            console.log(`Inserted ${insRes.insertedCount} documents with IDs: ${JSON.stringify(insRes.insertedIds)}.`);
        }

        // get all posts
        const posts = await db.collection<Post>(collection).find().toArray();
